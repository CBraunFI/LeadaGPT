...
const MeinUnternehmen = () => {
  ...
  const loadDocuments = async () => {
    try {
      setIsLoading(true);
      const data = await documentsAPI.getAll('company');
      setDocuments(data);
    } catch (error) {
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await documentsAPI.upload(file, 'company');
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">Mein Unternehmen</h1>
        <p style={{ color: 'var(--fg-secondary)' }}>
          Laden Sie Unternehmens-Dokumente hoch (Führungsleitlinien, Unternehmenswerte, Richtlinien),
          die Ihr KI-Coach bei der Beratung berücksichtigen soll.
        </p>
      </div>
      ...
      >
        ...
        <ul className="list-disc list-inside space-y-1">
          <li>Ihr KI-Coach berücksichtigt diese Dokumente bei Empfehlungen</li>
          <li>Ideal für Führungsleitlinien, Unternehmenswerte, Compliance-Richtlinien</li>
...
export default MeinUnternehmen;
